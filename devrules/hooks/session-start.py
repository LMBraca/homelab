...
def main() -> None:
    ...
    header = (
        ...
        "every devrules MCP tool call. Your context is stored remotely (not in "
        "this repo); read it below and append to it before you finish.\n\n"
        "**If the devrules tools (e.g. `context_append`) are NOT available to "
        "you, tell the user immediately** — this session isn't connected to "
        "devrules (it likely started before setup, or under a profile without "
        "the MCP registered), so nothing will be journaled until it's restarted "
        "under a wired profile. Don't silently skip the journal.\n\n"
    )
